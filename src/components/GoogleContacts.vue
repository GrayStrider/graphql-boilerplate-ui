<template lang=pug>
	v-app#inspire
		...
		v-app-bar(:clipped-left='$vuetify.breakpoint.lgAndUp' app color='blue darken-3' dark)
			...
			v-text-field.hidden-sm-and-down(flat solo-inverted hide-details prepend-inner-icon='mdi-magnify' label='Search')
				v-spacer
					v-switch(hide-details label='Dark Mode' v-model='$vuetify.theme.dark')
					v-btn(icon)
						v-icon mdi-apps
					v-btn(icon)
						v-icon mdi-bell
		...
		v-content
			v-container.fill-height(fluid)
				v-row(align='center' justify='center')
					ShipsTable
		v-btn(bottom right fixed color='pink' dark fab @click='dialog = !dialog')
			v-icon mdi-plus
